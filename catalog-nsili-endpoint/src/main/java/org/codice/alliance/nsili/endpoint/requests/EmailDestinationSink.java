...
public class EmailDestinationSink implements DestinationSink {
  ...
  public void writeFile(
      InputStream fileData, long size, String name, String contentType, List<Metacard> metacards)
      throws IOException {
    emailConfiguration
        .getEmailSender()
        .sendEmail(
  ...
  private String replaceIds(String bodyTemplate, List<Metacard> metacards) {
    return bodyTemplate.replace(
        METACARD_IDS, metacards.stream().map(Metacard::getId).collect(Collectors.joining(", ")));
  }

  private String replaceTitles(String bodyTemplate, List<Metacard> metacards) {
    return bodyTemplate.replace(
        METACARD_TITLES,
        metacards.stream().map(Metacard::getTitle).collect(Collectors.joining(", ")));
  }
}
